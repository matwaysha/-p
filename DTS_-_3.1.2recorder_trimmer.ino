...
LiquidCrystal_PCF8574 lcd(0x27); //using displey for checking and trimming
...
bool cn = false;
bool ct = false;
bool LCD = false;//trimming flags
bool LCd = false;
bool lail = false;
bool rail = false;
bool elev = false;
const int pot = A3;    //working with pot routine
const int pow_pin = A2;
const int POW_pin = A1; //working with lcd routine
const int button = A0;
...
void setup() {
  ...
  pinMode(Elev, OUTPUT);
  Serial.begin(115200);
  pinMode(pow_pin, OUTPUT);  //working with pot routine
  digitalWrite(pow_pin, HIGH);
   pinMode(POW_pin, OUTPUT);  //working with lcd routine
  digitalWrite(POW_pin, HIGH);
  lcd.begin(16, 2);      
  lcd.home(); lcd.clear();
  lcd.print("good day");
  ...
  ElevS.attach(Elev);
   ...
   digitalWrite(POW_PIN, HIGH);
    if (!SD.begin(CS_PIN))  // sd card connection checkup
{ 
...
void loop() {
  g = analogRead(button);
  if((g > 550) && (g < 560) && (LCD != true)){
    LCd = true;
    lcd.clear();
    lcd.print("trimmer is on");
    delay(2000);
  }
  if((g > 890) && (g < 900) && LCd){
    LCD = true;
  ...
  if(LCD){
    ...
    if((g > 220) && (g < 230)){
     ...
     lcd.clear();
     lcd.home();
    val = (analogRead(pot) * 0.0197) - 10;
    if(rail){
      lcd.print("right ail trim");
      lcd.setCursor(0,1);
      lcd.print(val);
      m = val;
    }
    if(lail){
      lcd.print("left ail trim");
      lcd.setCursor(0,1);
      lcd.print(val);
      n = val;
    }
